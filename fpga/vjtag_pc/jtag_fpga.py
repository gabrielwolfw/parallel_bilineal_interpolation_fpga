...
DATA_WIDTH = 8
ADDR_WIDTH = 18  # Para SETADDR (dirección completa de 18 bits - 256KB direccionables)
...
def set_address_to_fpga(conn, address):
    """Establece la dirección de memoria actual en la FPGA (18 bits - dirección completa)."""
    if conn is None:
        print_message(VERBOSE_LEVEL_QUIET, "|ERROR| No connection to server. Cannot set address.")
        return False
    try:
        # IMPORTANTE: El servidor TCL ahora espera ADDR_WIDTH bits (18 bits) para SETADDR
        addr_18bit = address & 0x3FFFF  # Máscara de 18 bits (256KB)
        binary_normal = format(addr_18bit, f'0{ADDR_WIDTH}b')  # 18 bits
        
        print_message(VERBOSE_LEVEL_DEBUG, f"|DEBUG| Setting address: {address} -> {addr_18bit} (18-bit complete)")
        print_message(VERBOSE_LEVEL_DEBUG, f"|DEBUG| Address binary ({ADDR_WIDTH} bits): {binary_normal}")
        
        # Enviar comando SETADDR seguido de la dirección en binario (18 bits completos)
        request = f"SETADDR {binary_normal}\n"
        print_message(VERBOSE_LEVEL_DEBUG, f"|DEBUG| Sending to TCL server: {request.strip()}")
        conn.sendall(request.encode())
        
        time.sleep(0.05)  # Pequeña pausa para estabilidad
        print_message(VERBOSE_LEVEL_NORMAL, f"|INFO| Address set to 0x{addr_18bit:05X}")
        return True
...
def main_loop(conn):
    ...
    print_message(VERBOSE_LEVEL_NORMAL, f"  setaddr <address>     - Set memory address (0x00000-0x{MAX_ADDR:05X})")
    ...
    print_message(VERBOSE_LEVEL_NORMAL, f"\nNote: Data values 0-{MAX_VAL} (0x00-0x{MAX_VAL:02X}), addresses 0x00000-0x{MAX_ADDR:05X} (256KB)")
        ...
        while True:
            ...
            if command == 'exit' or command == 'quit':
                ...
                if len(parts) == 2:
                    ...
                    if address is not None:
                        if 0 <= address <= MAX_ADDR:
                            print_message(VERBOSE_LEVEL_NORMAL, 
                                f"|INFO| Setting address to: {address} (0x{address:05X})")
                ...
                if len(parts) == 2:
                    ...
                    if address is not None:
                        if 0 <= address <= MAX_ADDR:
                            print_message(VERBOSE_LEVEL_NORMAL, f"|INFO| Reading from address: {address} (0x{address:05X})")
...
if __name__ == "__main__":
    ...
    print_message(VERBOSE_LEVEL_NORMAL, f"|CONFIG| Address width: {ADDR_WIDTH} bits (256KB addressable)")
