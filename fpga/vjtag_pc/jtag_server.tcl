...
set VJTAG_DATA_WIDTH 8 ; # Ancho de datos para READ/WRITE (8 bits)
set VJTAG_ADDR_WIDTH 18 ; # Ancho de dirección para SETADDR (18 bits - dirección completa)
...
puts "|INFO| VJTAG_DATA_WIDTH=$VJTAG_DATA_WIDTH, VJTAG_ADDR_WIDTH=$VJTAG_ADDR_WIDTH, TCP PORT=$server_port_number"
...
proc IncomingData {sock} {
    ...
    if {[eof $sock] || [catch {gets $sock line}]} {
        ...
        switch -exact -- $cmd {
            ...
            "SETADDR" {
                set addr [lindex $parts 1]
                # SETADDR usa VJTAG_ADDR_WIDTH (15 bits), no VJTAG_DATA_WIDTH
                if {[string length $addr] == $VJTAG_ADDR_WIDTH && 
                    [regexp "^\[01\]\{$VJTAG_ADDR_WIDTH\}$" $addr]} {
                    transmit_to_fpga_addr $addr
                } else {
                    puts "|ERROR TCL| Invalid SETADDR format: $addr (expected $VJTAG_ADDR_WIDTH bits, got [string length $addr] bits)"
                }
            }
            default {
...
proc transmit_to_fpga_addr {addr_binary_str} {
    global VJTAG_ADDR_WIDTH
    global IR_SET_ADDR IR_BYPASS
    
    # Debug: Muestra el string binario recibido
    puts "|DEBUG TCL| Received address binary string: $addr_binary_str"
    puts "|DEBUG TCL| IR Value: $IR_SET_ADDR (SETADDR)"
    
    # Convierte a decimal y muestra
    if {[regexp {^[01]+$} $addr_binary_str]} {
        set decimal_value 0
        set power 1
        for {set i [expr {[string length $addr_binary_str] - 1}]} {$i >= 0} {incr i -1} {
            if {[string index $addr_binary_str $i] == "1"} {
                incr decimal_value $power
            }
            set power [expr {$power * 2}]
        }
        puts "|DEBUG TCL| Address decimal value: $decimal_value"
        puts "|DEBUG TCL| Address hex value: [format "0x%04X" $decimal_value]"
    } else {
        puts "|ERROR TCL| Invalid binary address string received"
        return
    }
    ...
    device_virtual_dr_shift -dr_value $addr_binary_str -instance_index 0 -length $VJTAG_ADDR_WIDTH -no_captured_dr_value
    device_virtual_ir_shift -instance_index 0 -ir_value $IR_BYPASS -no_captured_ir_value
    closeport
}
...
proc read_from_fpga_address {addr_bin_str} {
    global VJTAG_DATA_WIDTH VJTAG_ADDR_WIDTH
    global IR_SET_ADDR IR_READ IR_BYPASS
    # Primero, setea la dirección usando IR_SET_ADDR con VJTAG_ADDR_WIDTH bits
    openport
    device_lock -timeout 10000
    # Setea la dirección (15 bits)
    device_virtual_ir_shift -instance_index 0 -ir_value $IR_SET_ADDR -no_captured_ir_value
    device_virtual_dr_shift -dr_value $addr_bin_str -instance_index 0 -length $VJTAG_ADDR_WIDTH -no_captured_dr_value
    # Ahora, ejecuta un IR_READ y lee el valor (8 bits)
    device_virtual_ir_shift -instance_index 0 -ir_value $IR_READ -no_captured_ir_value
    set tdo_hex_value [device_virtual_dr_shift -instance_index 0 -length $VJTAG_DATA_WIDTH -value_in_hex]
    # Regresa a bypass
    device_virtual_ir_shift -instance_index 0 -ir_value $IR_BYPASS -no_captured_ir_value
